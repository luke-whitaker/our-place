...
import { parseVideoUrl, ACCEPTED_IMAGE_TYPES, ACCEPTED_VIDEO_TYPES, MAX_IMAGE_SIZE, MAX_VIDEO_SIZE, formatFileSize } from '@/lib/media-utils';
...
export default function CreatePostForm({
      ...
      if (postType === 'rich') {
        ...
        const serializedBlocks = richBlocks
          ...
          .map(b => {
            ...
            if (b.type === 'video') return { type: 'video', url: b.url, media_source: b.media_source };
            return b;
          });
  ...
  function canSubmit(): boolean {
    ...
    if (postType === 'rich') {
      ...
      const hasContent = richBlocks.some(b =>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-medium text-gray-500">
            {isProfileMode ? 'Share something to My Place...' : 'Share something with the community...'}
          </span>
        </div>
      </button>
    );
            ...
            {!video ? (
                ...
                {videoMode === 'upload' ? (
                  <>
                    <input
                      ...
                      className="hidden"
                      onChange={(e) => { handleVideoFileSelect(e.target.files); e.target.value = ''; }}
                    />
                    <button
                      type="button"
              ...
              <div className="relative">
                {video.media_source === 'upload' ? (
                  <video src={video.url} controls className="w-full rounded-xl" style={{ maxHeight: '300px' }} />
                ) : (
      ...
      <div className="border-t border-gray-100 bg-gray-50 px-5 py-3">
        ...
        <div className="flex items-center justify-between">
          <div className="text-xs text-gray-400">
            ...
            {postType === 'video' && video && (video.media_source === 'upload' ? 'Video attached' : `${video.media_source} embed`)}
