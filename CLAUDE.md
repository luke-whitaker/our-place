...
- **R2 uploads must send `Content-Length` explicitly.** R2 has no chunked-upload support and answers `411 MissingContentLength` without the header. undici streams any body at or above its 64 KiB high-water mark, and a streamed body loses the `Content-Length` that `fetch` would otherwise derive, so `src/lib/storage.ts` and `scripts/backup-db.ts` both set it from the body length. Omitting it fails **only** above that mark, which is why a tiny fixture passes the exact code path a real photo fails on. Test media and payload paths at realistic sizes.
